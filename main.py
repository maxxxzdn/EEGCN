...
parser.add_argument('--path_data', type=str, default='../EEG_data/data/*/*/*/*')
...
while train_acc < 0.99:
    ...
    for i in range(0,len(train_dataset),args.batch_size):
        loss += train(model, optimizer, train_dataset[i:(i+args.batch_size)], device)
        with torch.no_grad():
            MSE_train += test(model, train_dataset[i:(i+args.batch_size)], device)
    with torch.no_grad():
